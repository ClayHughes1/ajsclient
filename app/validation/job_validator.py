...
class JobValidator:
    ...
    def validate(self, job) -> tuple[bool, str]:
        ...
        # IMPORTANT:
        #
        # "United States" is a generic location and must only
        # match when the entire location is "United States".
        #
        # Otherwise a location such as:
        #
        # "Orlando, FL United States"
        #
        # would incorrectly match "United States" as a substring.
        #
        # All other accepted locations retain the existing
        # substring-matching behavior.
        # ---------------------------------------------------------

        if job_location == "united states":
            return True, ""

        for accepted_location in accepted_locations:

            accepted_location_lower = (
                accepted_location.lower().strip()
            )

            # The generic "United States" entry is handled
            # separately above and must not be used as a
            # substring match.
            if accepted_location_lower == "united states":
                continue

            if accepted_location_lower in job_location:
                return True, ""
